...
export const metadata = { title: "Canais — AmericaSim", robots: { index: false, follow: false } };
...
export default async function CanaisDoProduto({ params }: { params: Promise<{ handle: string }> }) {
  ...
  const linhas: Linha[] = vars.rows.map((v: any) => {
    ...
    return {
      ...
      custoMoeda: v.custo_moeda, custoBrl: v.custo_brl, fonteCusto: v.fonte_custo,
      disponivel: Number(v.disponivel ?? 0), celulas,
    };
  });
  ...
  return (
    <>
      ...
      <Abas handle={prod.handle} atual="canais" />
              ...
              <tbody>
                {hist.rows.map((h: any, i: number) => (
                  <tr key={i} style={{ borderTop: "1px solid var(--borda)" }}>
                    ...
                    <td style={{ padding: "10px 16px", textAlign: "right" }}>R$ {String(h.valor).replace(".", ",")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
